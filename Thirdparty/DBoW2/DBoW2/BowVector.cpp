...
{
  //std::cout << "bowvec normalize type is "<< norm_type << std::endl;
  double norm = 0.0;
  BowVector::iterator it;

  if(norm_type == DBoW2::L1)
  {
    //std::cout << "bowvec L1 normalize" << std::endl;
    for(it = begin(); it != end(); ++it)
      norm += fabs(it->second);
  }
  else
  {
    //std::cout << "bowvec L2 normalize" << std::endl;
    for(it = begin(); it != end(); ++it)
  ...
  if(norm > 0.0)
  {
    //std::cout << "into normalization" << std::endl;
    for(it = begin(); it != end(); ++it)
      it->second /= norm;
  }
}

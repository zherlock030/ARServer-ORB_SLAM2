//
// Created by 范宏昌 on 2020/1/5.
//

#ifndef ORB_SLAM2_TWOLINEIN3D_H
#define ORB_SLAM2_TWOLINEIN3D_H
...
public:
    ...
    void GetDistance() {
        ...
        Pon_x=(PonA_x+PonB_x)/2.;
        Pon_y=(PonA_y+PonB_y)/2.;
        Pon_z=(PonA_x+PonB_z)/2.;
...
private:
    ...
    //点乘
    static double dot(double ax, double ay, double az, double bx, double by, double bz) { return ax * bx + ay * by + az * bz; }

    //向量叉乘得到法向量，最后三个参数为输出参数
    static void cross(double ax, double ay, double az, double bx, double by, double bz, double &x, double &y, double &z) {
        x = ay * bz - az * by;
        y = az * bx - ax * bz;
        z = ax * by - ay * bx;
    }
    ...
    //向量取模
    static double norm(double ax, double ay, double az) { return sqrt(dot(ax, ay, az, ax, ay, az)); }

};


#endif //ORB_SLAM2_TWOLINEIN3D_H
